...
Каждый прогон печатает текущий статус каждой заявки (`state` как есть, без
перевода). Если статус изменился с прошлого запуска — добавляется `(было …)`.
Прошлые статусы хранятся в `state.json` рядом с проектом.
